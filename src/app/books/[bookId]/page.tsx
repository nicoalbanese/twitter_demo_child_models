...
import { getAuthors } from "@/lib/api/authors/queries";import OptimisticBook from "@/app/books/[bookId]/OptimisticBook";
...
const Book = async ({ id }: { id: string }) => {
  ...
  const { book, quotes, reflections } = await getBookByIdWithQuotesAndReflections(id);
  ...
  return (
    <Suspense fallback={<Loading />}>
      ...
      <div className="relative mt-8 mx-4">
        <h3 className="text-xl font-medium mb-4">{book.title}&apos;s Reflections</h3>
        <ReflectionList
          books={[]}
          bookId={book.id}
